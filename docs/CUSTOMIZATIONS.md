...
Some example data is provided in the `fixtures` folder, e.g. `additionallicenses.json` includes [creative-commons licenses](https://creativecommons.org/).
...
However, you can configure (to some extend) available components via `geonode-mapstore-client/_geonode_config.html`

If you make UI changes which requires a `geonode-mapstore-client` fork building the GeoNode image can be cumbersome.
...
As a federal research institute, metadata has to comply with the GDI-DE metadata schema which is a profile of ISO 19135.
